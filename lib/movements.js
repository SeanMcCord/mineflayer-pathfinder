...
class Movements {
  ...
  getBlock(pos, dx, dy, dz, node) {
    ...
    if (this.allowMutationHistory) {
      blockState = node.mutatedBlockStateMap.get(blockPosition.toString())
    }
    let b
    if (blockState != null) {
      b = {
  ...
  getMoveJumpUp(node, dir, neighbors) {
    ...
    neighbors.push(new Move(blockB.position.x, blockB.position.y, blockB.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
  }
  ...
  getMoveForward(node, dir, neighbors) {
    ...
    neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
  }
  ...
  getMoveDiagonal(node, dir, neighbors) {
    ...
    if (y === 1) {
      ...
      cost += 1
      neighbors.push(new Move(blockC.position.x, blockC.position.y + 1, blockC.position.z, node.remainingBlocks, cost, toBreak, undefined, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
    } else if (blockD.physical || blockC.liquid) {
      // Case on same level
      neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks, cost, toBreak, undefined, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
  ...
  getMoveDropDown(node, dir, neighbors) {
    ...
    neighbors.push(new Move(blockLand.position.x, blockLand.position.y, blockLand.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
  }
  ...
  getMoveUp(node, neighbors) {
    ...
    neighbors.push(new Move(node.x, node.y + 1, node.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
  }
  ...
  getMoveParkourForward(node, dir, neighbors) {
    ...
    for (let d = 2; d <= maxD; d++) {
      ...
      if (ceilingClear && blockB.safe && blockC.safe && blockD.physical) {
        ...
        // Forward
        neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks, 1, [], [], true, positionsOccupiedForMove, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
        break
        ...
        if (blockA.safe) {
          ...
          neighbors.push(new Move(blockB.position.x, blockB.position.y, blockB.position.z, node.remainingBlocks, 1, [], [], true, positionsOccupiedForMove, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
          break
        }
        ...
        if (blockE.physical) {
          ...
          neighbors.push(new Move(blockD.position.x, blockD.position.y, blockD.position.z, node.remainingBlocks, 1, [], [], true, positionsOccupiedForMove, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
        }
  ...
  getPlaceBlock(node, dir, yOffset, neighbors) {
    ...
    cardinalDirectionVectors.some((direction) => {
      ...
      if (refBlock.physical) {
        ...
        neighbors.push(new Move(node.x, node.y, node.z, node.remainingBlocks - toPlace.length, cost, undefined, toPlace, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.costToCome, node.priorPosHashes))
        return true
      } else {
        return false
      }
  ...
  *getRandomNeighborsGenerator(node) {
    ...
    neighborSuppliers.push((neighbors) => this.getMoveDown(node, neighbors));
    ...
    for (const neighborSupplier of neighborSuppliers) {
      const neighbors = [];
