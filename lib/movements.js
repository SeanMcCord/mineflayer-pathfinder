...
class Movements {
  ...
  getBlock(posX, posY, posZ, dx, dy, dz, node) {
    ...
    const blockPosition = new Vec3(posX + dx, posY + dy, posZ + dz)
    // Get the state after all previous mutations
    // let blockState = null;
    // if (this.allowMutationHistory) {
    const blockState = node.mutations.getPos(blockPosition)
    // console.log({blockPosition, blockState});
    // }
    if (blockState != null) {
      return {
    ...
    return {
      ...
      shapes: b.shapes,
      digTime: () => b.digTime(),
      replaceable: this.replaceablesArray[b.type] && !physical,
      safe: (b.boundingBox === 'empty' || climbable || this.carpetsArray[b.type]) && !this.blocksToAvoidArray[b.type],
      physical,
    ...
    //  b.height = posY + dy
    //  for (const shape of b.shapes) {
    //    b.height = Math.max(b.height, posY + dy + shape[4])
    //  }
    //  return b
  }
  ...
  safeToBreak(block, node) {
    ...
    if (this.dontCreateFlow) {
      ...
      if (this.getBlock(block.position.x, block.position.y, block.position.z, 0, 1, 0, node).liquid) return false
      if (this.getBlock(block.position.x, block.position.y, block.position.z, -1, 0, 0, node).liquid) return false
      if (this.getBlock(block.position.x, block.position.y, block.position.z, 1, 0, 0, node).liquid) return false
      if (this.getBlock(block.position.x, block.position.y, block.position.z, 0, 0, -1, node).liquid) return false
      if (this.getBlock(block.position.x, block.position.y, block.position.z, 0, 0, 1, node).liquid) return false
    }
  ...
  getMoveJumpUp(node, dir, neighbors) {
    const blockA = this.getBlock(node.x, node.y, node.z, 0, 2, 0, node)
    const blockH = this.getBlock(node.x, node.y, node.z, dir.x, 2, dir.z, node)
    const blockB = this.getBlock(node.x, node.y, node.z, dir.x, 1, dir.z, node)
    const blockC = this.getBlock(node.x, node.y, node.z, dir.x, 0, dir.z, node)
    ...
    if (!blockC.physical) {
      ...
      const blockD = this.getBlock(node.x, node.y, node.z, dir.x, -1, dir.z, node)
      if (!blockD.physical) {
    ...
    const block0 = this.getBlock(node.x, node.y, node.z, 0, -1, 0, node)
  ...
  getMoveForward(node, dir, neighbors) {
    ...
    if (this.getBlock(node.x, node.y, node.z, 0, 0, 0, node).liquid) cost += this.liquidCost
  ...
  getMoveDiagonal(node, dir, neighbors) {
    ...
    const blockB1 = this.getBlock(node.x, node.y, node.z, 0, y + 1, dir.z, node)
    // Block feet will move into if going in z
    const blockC1 = this.getBlock(node.x, node.y, node.z, 0, y, dir.z, node)
    ...
    const blockB2 = this.getBlock(node.x, node.y, node.z, dir.x, y + 1, 0, node)
    // Block feet will move into if going in x
    const blockC2 = this.getBlock(node.x, node.y, node.z, dir.x, y, 0, node)
    ...
    const blockStandInGoal = this.getBlock(node.x, node.y, node.z, dir.x, y, dir.z, node)
    cost += this.safeOrBreak(blockStandInGoal, toBreak, node)
    if (cost > 100) return
    const blockHeadInGoal = this.getBlock(node.x, node.y, node.z, dir.x, y + 1, dir.z, node)
    ...
    if (y === 1) {
      ...
      const block0 = this.getBlock(node.x, node.y, node.z, 0, -1, 0, node)
      if (blockC.height - block0.height > 1.2) return // Too high to jump
      const blockAbove = this.getBlock(node.x, node.y, node.z, 0, 2, 0, node)
    ...
    } else if (this.getBlock(node.x, node.y, node.z, dir.x, -2, dir.z, node).physical || blockD.liquid) {
  ...
  getLandingBlock(node, dir) {
    ...
    let blockLand = this.getBlock(node.x, node.y, node.z, dir.x, -2, dir.z, node)
    ...
    while (blockLand.position && blockLand.position.y > 0) {
      ...
      if (blockLand.physical) {
        if (node.y - blockLand.position.y <= this.maxDropDown) {
          ...
          const blockAboveLanding = this.getBlock(blockLand.position.x, blockLand.position.y, blockLand.position.z, 0, 1, 0, node)
      ...
      if (!blockLand.safe) {
        ...
        blockLand = this.getBlock(blockLand.position.x, blockLand.position.y, blockLand.position.z, 0, -1, 0, node)
        // positionsOccupied.push(blockLand.position)
      }
    }
    return {block: null, positions: []}
  ...
  getMoveDown(node, neighbors) {
    ...
    if (this.getBlock(node.x, node.y, node.z, 0, 0, 0, node).liquid) return // dont go underwater

    neighbors.push(new Move(blockLand.position.x, blockLand.position.y, blockLand.position.z, node.remainingBlocks - toPlace.length, cost, toBreak, toPlace, undefined, positionsOccupied, node.allBroken, node.allPlaced, node.actionChain, node.mutatedBlockStateMap, node.costToCome, node.priorPosHashes, node.mutations))
  }

  getMoveUp(node, neighbors) {
    const block1 = this.getBlock(node.x, node.y, node.z, 0, 0, 0, node)
    if (block1.liquid) return

    const block2 = this.getBlock(node.x, node.y, node.z, 0, 2, 0, node)
    let cost = 1 // move cost
    const toBreak = []
    const toPlace = []
    const positionsOccupied = []
  ...
  getMoveParkourForward(node, dir, neighbors) {
    ...
    const block1 = this.getBlock(node.x, node.y, node.z, dir.x, -1, dir.z, node)
    const blockFeetA = this.getBlock(node.x, node.y, node.z, dir.x, 0, dir.z, node)
    const blockHeadA = this.getBlock(node.x, node.y, node.z, dir.x, 1, dir.z, node)
    ...
    if (this.getBlock(node.x, node.y, node.z, 0, 0, 0, node).liquid) return // cant jump from water

    // If we have a block on the ceiling, we cannot jump but we can still fall
    const ceilingBlockA = this.getBlock(node.x, node.y, node.z, 0, 2, 0, node)
    const ceilingBlockB = this.getBlock(node.x, node.y, node.z, dir.x, 2, dir.z, node)
    ...
    const floorBlock = this.getBlock(node.x, node.y, node.z, dir.x, -2, dir.z, node)
    ...
    for (let d = 2; d <= maxD; d++) {
      const dx = dir.x * d
      const dz = dir.z * d
      const blockA = this.getBlock(node.x, node.y, node.z, dx, 2, dz, node)
      const blockB = this.getBlock(node.x, node.y, node.z, dx, 1, dz, node)
      const blockC = this.getBlock(node.x, node.y, node.z, dx, 0, dz, node)
      const blockD = this.getBlock(node.x, node.y, node.z, dx, -1, dz, node)
      ...
      if (ceilingClear && blockB.safe && blockC.safe && blockD.physical) {
        ...
        // Down
        const blockE = this.getBlock(node.x, node.y, node.z, dx, -2, dz, node)
        if (blockE.physical) {
          const positionsOccupiedForMove = [];
  ...
  getPlaceBlock(node, dir, yOffset, neighbors) {
    ...
    const blockBOrC = this.getBlock(node.x, node.y, node.z, dir.x, yOffset, dir.z, node)
    ...
    cardinalDirectionVectors.some((direction) => {
      ...
      const refBlock = this.getBlock(targetBlockPos.x, targetBlockPos.y, targetBlockPos.z, direction.x, direction.y, direction.z, node)
